...
function setParams(filterData){
  ...
  var params = "";
  var pageData = { page: page, pagelimit: pagelimit, category: 'shoes'};
...
if(data.success){
    ...
    function fillData(item, index){
        ...
        imgUrl = baseURL + '/swatches/shoes/' + item.ID + '.jpg';
        ...
        var myEscapedJSONString = jsonStr
         ...
         _Html += '<div class="hover-btns"></div><img class="center-block"  data-value='+myEscapedJSONString+' src="'+imgUrl+'"></a><span>'+item.ID+'</span></div></div>';
          ...
          $('#shoes_list .clearfix').append(_Html);

          if(index == ( data.success.fetched - 1))
          {
            $('#shoes_list .clearfix').imagesLoaded( function() {
      ...
      $('#shoes_list .clearfix').append('<div style="margin-left:45%; margin-top:50px;">'+data.error+'</div>');
      $('.preloader-wrapper').hide();
    }
  ...
  $('#shoes_list').on('click','.center-block',function(){
            $('#fabric_rightside').find('.sidebar-toggle.shoes-menu').trigger( "click" );
            var url =  $(this).attr('src');
            var dictionary = $(this).data('value');
            
            //main call
            sceneEl.emit('changeTexture',  {src: url, part:'shoes', entity:'shoesEntity', data: dictionary}, false);
            
            $(this).parents('#shoes_list').find('.load_fabric').removeClass('active');
            $(this).parents('#shoes_list .load_fabric').addClass('active');

    });


  $('#shoes_list').on('scroll', function() {
