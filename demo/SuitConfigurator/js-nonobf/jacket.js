...
if(data.success){
    ...
    function fillData(item, index){
        ...
        var myEscapedJSONString = jsonStr
         ...
         _Html += '<div class="hover-btns"></div><img class="center-block"  data-value='+myEscapedJSONString+' src="'+imgUrl+'"></a><span>'+item.ID+'</span></div></div>';
          ...
          $('#jacket_list .clearfix').append(_Html);

          if(index == ( data.success.fetched - 1))
          {
            $('#jacket_list .clearfix').imagesLoaded( function() {
      ...
      $('#jacket_list .clearfix').append('<div style="margin-left:45%; margin-top:50px;">'+data.error+'</div>');
      $('.preloader-wrapper').hide();
    }
  ...
  $('#jacket_list').on('click','.center-block',function(){
            $('#fabric_rightside').find('.sidebar-toggle.jacket-menu').trigger( "click" );
            var url =  $(this).attr('src');
            var dictionary = $(this).data('value');
            
            //main call
            sceneEl.emit('changeTexture', {src: url, part:'jacket', entity:'jacketEntity', extraenitity: ['lapelEntity', 'pocketEntity', 'ventEntity'], data: dictionary}, false);

            $(this).parents('#jacket_list').find('.load_fabric').removeClass('active');
            $(this).parents('#jacket_list .load_fabric').addClass('active');

    });


  $('#jacket_list').on('scroll', function() {
...
function setFilter(){
  ...
  $('#filterJacketData').click(function () {
  ...
  var filter = $("#jacketFilterForm").serializeArray(); // Serialize form Data
...
function setFilterOptions(){
  ...
  $('#jacketbook').multiselect();
  $('#jacketpattern').multiselect();
  ...
  dataType:'json',
    success : function(data) {  
        ...
        $('#jacketbook').multiselect('loadOptions', bookopts);
        ...
        $('#sjacketpattern').multiselect('loadOptions', patternopts);
